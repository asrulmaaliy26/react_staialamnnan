  ...
  useEffect(() => {
    const loadLevelConfig = async () => {
      try {
        ...
        setConfigError(error.message || 'Gagal memuat konfigurasi sekolah. Pastikan server API berjalan.');
      }
    };
    loadLevelConfig();
  }, []);
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-slate-50">
        <div className="text-center p-8 bg-white rounded-2xl shadow-lg border border-red-100 max-w-md mx-4">
          ...
          <h2 className="text-xl font-bold text-slate-800 mb-2">Gagal Terhubung ke Server</h2>
          <p className="text-slate-600 text-sm mb-4">{configError}</p>
          <button 
